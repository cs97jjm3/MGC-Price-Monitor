...
class PriceDatabase {
  ...
  async initialize() {
    ...
    this.db.run(`CREATE INDEX IF NOT EXISTS idx_failure_car_id ON scrape_failures(car_id)`);
  ...
  logFailure(carId, errorType, errorMessage, htmlSnapshot = null) {
    this.db.run(`
      INSERT INTO scrape_failures (car_id, error_type, error_message, html_snapshot) 
      VALUES (?, ?, ?, ?)
    `, [carId, errorType, errorMessage, htmlSnapshot]);
    this.save();
  }

  getConsecutiveFailures(carId) {
  ...
  getRecentFailures(hours = 24) {
    ...
    const result = this.db.exec(`
      SELECT f.car_id, c.name, c.url, f.error_type, f.error_message, f.failed_at
  ...
  clearOldFailures(carId) {
    // Clear old failures when a successful check happens
    this.db.run('DELETE FROM scrape_failures WHERE car_id = ?', [carId]);
    this.save();
  }

  getCarsWithPersistentFailures(days = 7) {
    const cutoff = new Date();
    ...
    const result = this.db.exec(`
      ...
      FROM cars c
      JOIN scrape_failures f ON c.id = f.car_id
      WHERE f.failed_at > ?
      AND NOT EXISTS (
